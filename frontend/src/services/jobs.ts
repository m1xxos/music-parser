...
export async function submitBatch(payload:{urls:string[];trim?:{startSeconds?:number;endSeconds?:number};metadata?:{artist?:string;album?:string}}){const r=await fetch('/api/v1/batch',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!r.ok)throw new Error('batch submit failed');return r.json()}
